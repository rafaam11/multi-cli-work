...
**Exception — session rows stay neutral, not brand-colored.** In `ProjectSidebar.tsx`, `.session-row`'s own `color` is fixed to `var(--muted)` (`index.css:502`) regardless of status — the per-session `--session-accent` variable drives the row's background wash and left box-shadow bar (and the separate `.status-dot` element), not the row's `color`/the icon's inherited `currentColor`. So the provider icon was never dynamically status-tinted by hue; the status signal a user reads at a glance is the background wash, bar, and dot, not the icon's color. Applying a fixed brand color to the icon there would still be wrong, though for a different reason: it would make the icon the one element in the row not sharing the row's neutral `--muted`/`--text` treatment, an inconsistency with every other icon in the same tree (folder icons, chevrons, the `Wrench` fallback for tool sessions). The brand-accent classes below are therefore applied only at static call sites outside this row; the session-row icon swap is shape-only.
