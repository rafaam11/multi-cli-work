...
export async function readGitStatus(rootPath: string): Promise<GitStatusResult> {
  try {
    const [branchResult, statusResult] = await Promise.all([
      runReadOnlyGit(["-C", rootPath, "rev-parse", "--abbrev-ref", "HEAD"], { windowsHide: true, timeout: 5_000 }),
      runReadOnlyGit(["-C", rootPath, "status", "--porcelain"], { windowsHide: true, timeout: 5_000 }),
    ]);
    return {
      isRepo: true,
