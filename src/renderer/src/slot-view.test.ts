...
describe("nextWorkspaceSlot", () => {
  ...
  it("fills a hole a drop reached past before opening a new page", () => {
    const shelf = [
      { layoutId: "cols:2-2-2", slots: ["a", null, "c", "d", "e", "f"] },
      { layoutId: "cols:2-2-2", slots: full("b") },
      { layoutId: "cols:2-2-2", slots: full("c") },
    ];
  ...
  it("counts a page by the workspace's own layout rather than a fixed six", () => {
    expect(nextWorkspaceSlot([{ layoutId: "cols:1-1", slots: ["a", "b"] }, auto(), auto()])).toEqual({
      index: 1,
      slot: 0,
    });
  });
