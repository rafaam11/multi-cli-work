...
- 왼쪽 트리에 **업무 프로젝트** > **열어둔 폴더** > 그 폴더의 **세션과 문서**. 폴더를 누르면 그 폴더의 패인이 오른쪽 **슬롯 그리드**를 채운다. 레이아웃(1~6칸)을 직접 고르고, 앱이 들고 있는 세션·문서는 폴더와 상관없이 **작업공간** 한 화면에 자동으로 모인다. 지금 안 보고 싶은 것은 **숨김**으로 옮긴다.
...
- **사이드바 = 작업공간 선반 + 폴더 트리** — 맨 위에 **작업공간**과 **숨김** 두 줄, 그 아래에 **업무 프로젝트 > 폴더 > 세션·문서** 세 겹의 트리. 폴더 이름 옆의 작은 숫자 칩은 그 폴더가 가진 세션 수(worktree 것 포함)다. 두 행 모두 펼치면 담고 있는 패인이 슬롯 순서로 나온다. **＋** 로 작업할 폴더를 열면 목록에 남고, 앱을 껐다 켜도 그대로다. 자동 발견은 하지 않는다 — **내가 연 폴더만** 보인다. 폴더 색은 **직접 켜는 것이 아니라** 그 폴더의 에이전트를 따라간다: 실제로 도는 세션이 하나라도 있으면 amber, 아니면 green.
...
- **작업공간과 숨김** — 사이드바 맨 위의 두 줄. **작업공간**에는 앱이 들고 있는 세션과 문서가 **전부 자동으로** 모인다 — 끌어다 놓을 필요도, 메뉴를 열 필요도 없고, 앱을 껐다 켜서 복원된 세션이나 다른 창이 만든 세션도 같은 자리로 들어온다(원본 폴더의 행은 그대로 남는 참조다). 화면은 그대로 두고 조용히 담기므로, 모아 놓은 화면을 보려면 작업공간 행을 누른다. 잠시 빼 두고 싶은 패인은 **✕** 로 **숨김**에 옮긴다 — 세션은 계속 돌고 문서도 열린 채다. 숨김도 같은 슬롯 그리드라 행을 누르면 숨긴 것만 모아 볼 수 있고, 거기서 다시 **✕** 를 누르면 작업공간으로 돌아온다. 세션 **우클릭 → 작업공간에서 숨기기 / 작업공간에 다시 표시** 도 같은 이동이다. 두 화면 다 자기 레이아웃을 기억하고, 행을 **펼치면** 담고 있는 패인이 슬롯 순서로 나와 화면 밖 페이지의 패인도 눌러서 데려올 수 있다. **모든 패인은 두 곳 중 정확히 한 곳에 있다.**
...
**여러 폴더를 한 화면에 (작업공간 · 숨김)** — 사이드바 맨 위의 **작업공간** 행에는 앱이 들고 있는 세션과 문서가 전부 자동으로 담긴다. 폴더가 달라도 상관없고, 원본 폴더의 행은 그대로 남는다. 행을 클릭하면 담아 둔 것들이 자기 레이아웃으로 뜨고, 행 앞의 펼침 버튼을 누르면 담긴 패인 목록이 슬롯 순서로 보인다. 지금 안 보고 싶은 것은 패인 헤더의 **✕(작업공간에서 숨기기)**, 펼친 목록의 **✕**, 또는 세션 **우클릭 → 작업공간에서 숨기기** 로 **숨김** 행에 옮긴다 — 세션은 계속 돌고 문서도 열린 채이며, 자동 수집이 이것을 되돌리지 않는다. 숨김 행을 누르면 숨긴 것들만 그리드에 뜨고, 거기서 **✕(작업공간에 다시 표시)** 를 누르면 되돌아온다.

**홈으로 돌아가기** — 좌측 상단 로고를 누르면 현재 선택을 건드리지 않고 홈 대시보드로 이동한다. 세션 모니터·빠른 실행·최근 활동 피드 어디서든 클릭 한 번으로 해당 세션으로 돌아갈 수 있다.

**세션 다루기** — 세션 조작은 **패인 헤더**에 있다. **⏹ 중지** 는 프로세스만 끄고 기록은 남기고(나중에 **⟲ 재개**), **✕ 슬롯 비우기** 는 세션을 살려 둔 채 그 칸만 비운다 — 사이드바 행은 남아 있으니 다시 누르면 돌아온다(작업공간·숨김 화면에서는 같은 자리의 ✕가 패인을 반대편으로 옮긴다). 세션 자체를 지우려면 **🗑 제거** 를 누른다 — **확인 없이 즉시 삭제되고 되돌릴 수 없다**. 패인 헤더의 ✕ 옆에 있고, 상단 헤더 오른쪽 끝에도 같은 버튼이 있어 지금 포커스된 패인의 세션을 지운다. 패인 헤더 **우클릭 → 제거** 도 그대로 동작한다. 폴더를 클릭하면 그 세션들이 그리드에 올라오고, 스크롤백은 앱이 꺼져 있던 동안 것도 함께 복원된다. 앱 종료로 중단됐던 세션이라면 이 첫 열람에서 대화가 자동으로 재개된다.
