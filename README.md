...
- **사이드바 = 작업공간 선반 + 폴더 트리** — 맨 위에 **작업공간1/2/3**, 그 아래에 **업무 프로젝트 > 폴더** 두 겹의 트리. 세션 행은 트리에 없다 — 세션은 폴더를 눌러 뜨는 탭 바와 슬롯 그리드에서 다룬다. **＋** 로 작업할 폴더를 열면 목록에 남고, 앱을 껐다 켜도 그대로다. 자동 발견은 하지 않는다 — **내가 연 폴더만** 보인다. 폴더 색은 **직접 켜는 것이 아니라** 그 폴더의 에이전트를 따라간다: 실제로 도는 세션이 하나라도 있으면 amber, 아니면 green.
...
- **문서도 패인이다** — 우측 사이드바에서 연 파일·diff·커밋 그래프·PR은 세션과 같은 자격으로 슬롯을 차지한다. 코드를 띄운 채 그 옆에서 에이전트를 굴리는 배치가 그리드 안에서 끝난다.
...
**화면 배치 정하기** — 그리드 위의 레이아웃 줄에서 **자동**과 1~6칸 프리셋 중 하나를 고른다. 고른 배치는 폴더마다(작업공간마다) 따로 기억된다. 세션이 칸 수보다 많으면 오른쪽의 **◀ 2/3 ▶** 로 페이지를 넘기거나, 탭을 눌러 그 세션이 있는 페이지로 바로 간다 — 화면 밖으로 밀린 세션도 계속 돌고 있고 스크롤백도 그대로다. 칸이 남으면 빈 칸이 드롭 자리로 남는다: 탭을 끌어다 놓으면 그 칸에 들어가고, 패인 헤더를 다른 칸으로 끌면 자리가 바뀐다.
...
**세션 다루기** — 세션 조작은 **패인 헤더**에 있다. **⏹ 중지** 는 프로세스만 끄고 기록은 남기고(나중에 **⟲ 재개**), **✕ 슬롯 비우기** 는 세션을 살려 둔 채 그 칸만 비운다 — 탭은 남아 있으니 다시 누르면 돌아온다. 세션 자체를 지우려면 패인 헤더를 **우클릭 → 제거**(확인 후 삭제). 폴더를 클릭하면 그 세션들이 그리드에 올라오고, 스크롤백은 앱이 꺼져 있던 동안 것도 함께 복원된다. 앱 종료로 중단됐던 세션이라면 이 첫 열람에서 대화가 자동으로 재개된다.
