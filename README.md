...
- 왼쪽 트리에 **업무 프로젝트**와 그 아래 **열어둔 폴더**. 폴더를 누르면 그 폴더의 세션이 오른쪽에 **탭 한 줄 + 슬롯 그리드**로 뜬다. 레이아웃(1~6칸)을 직접 고르고, 여러 폴더의 세션을 모아 보고 싶으면 탭을 **작업공간1/2/3**으로 끌어다 놓는다.
...
<p align="center"><em>왼쪽에 작업공간과 업무 프로젝트·폴더, 오른쪽에 그 폴더의 세션들이 탭 한 줄과 슬롯 그리드로. 화면에 없는 세션이 승인을 기다리기 시작하면 알림이 뜬다.</em></p>
...
- **사이드바 = 작업공간 선반 + 폴더 트리** — 맨 위에 **작업공간1/2/3**, 그 아래에 **업무 프로젝트 > 폴더** 두 겹의 트리. 세션 행도 문서 행도 트리에 없다 — 둘 다 폴더를 눌러 뜨는 탭 바와 슬롯 그리드에서 다룬다. **＋** 로 작업할 폴더를 열면 목록에 남고, 앱을 껐다 켜도 그대로다. 자동 발견은 하지 않는다 — **내가 연 폴더만** 보인다. 폴더 색은 **직접 켜는 것이 아니라** 그 폴더의 에이전트를 따라간다: 실제로 도는 세션이 하나라도 있으면 amber, 아니면 green.
...
- **읽지 않음 배지** — 알림과 같은 판정을 사이드바에도 그린다. 폴더 행에 하위 세션 중 가장 급한 상태의 점 배지(입력 대기=violet, 승인 대기=amber)가 올라온다. 작업표시줄 아이콘에는 오버레이 점, 트레이 툴팁에는 대기 세션 수가 함께 표시된다.
...
- **탭 바 + 슬롯 그리드** — 폴더를 누르면 그 폴더의 세션(종료된 세션의 스크롤백 포함)이 **탭 한 줄**로 서고 최근 활동순으로 그리드의 슬롯을 채운다. 배치는 **헤더 줄 왼쪽의 레이아웃 타일**에서 고르며(자동 + 프리셋 8개), **레이아웃이 곧 한 화면의 칸 수**다. 세션이 칸 수보다 많으면 **다음 페이지**로 넘어가고(탭을 누르면 그 세션이 있는 페이지로 이동), 적으면 빈 칸이 **드롭 자리**로 남는다. 패인 헤더를 끌어 슬롯끼리 자리를 바꾸고, **✕(슬롯 비우기)** 는 세션을 살려 둔 채 그 칸만 비운다. 기본값 **자동**은 세션 수에 맞춰 1=전체 · 2=좌우 · 3=3열 · 4=3열+우측 2분할 · 5=메인+우측 2×2 · 6=3×2로 배치한다.
...
- **작업공간1/2/3** — 사이드바 맨 위의 세 줄. 세션·문서 탭을 그 위로 끌어다 놓으면 **여러 폴더의 패인을 한 화면에** 모을 수 있다(원본 폴더의 탭은 그대로 남는 참조다). 작업공간도 폴더 뷰와 같은 껍데기(탭 바 + 슬롯 그리드)이고, 자기만의 레이아웃을 기억한다. 폴더 뷰와 달리 **자동으로 채워지지 않는다** — 손으로 담은 것만 들어 있다.
- **문서도 패인이다** — 우측 사이드바에서 연 파일·diff·커밋 그래프·PR은 세션과 같은 자격으로 슬롯을 차지한다. 코드를 띄운 채 그 옆에서 에이전트를 굴리는 배치가 그리드 안에서 끝난다. 문서는 **왼쪽 트리에 행을 만들지 않는다** — 세션과 마찬가지로 탭 바와 그리드에서만 다루고, 트리는 폴더와 worktree까지다.
...
**세션 만들기 · 상세 페이지** — 폴더를 클릭하면 그 폴더의 탭 바와 슬롯 그리드로 바로 간다. 헤더의 **PowerShell(Windows) 또는 Bash(Linux) · Claude Code · Codex** 버튼으로 세션을 시작하며(설치되지 않은 CLI는 비활성), 세션이 하나도 없으면 빈 그리드에 그 버튼들만 남는다. 세션 카드·git 상태·메모를 보려면 헤더의 **상세** 버튼을 누른다.

**화면 배치 정하기** — 헤더 줄 왼쪽의 레이아웃 타일에서 **자동**과 프리셋 8개(1칸 · 2칸 좌우 · 3칸 2종 · 4칸 2종 · 5칸 · 6칸) 중 하나를 고른다. 고른 배치는 폴더마다(작업공간마다) 따로 기억된다. 세션이 칸 수보다 많으면 오른쪽의 **◀ 2/3 ▶** 로 페이지를 넘기거나, 탭을 눌러 그 세션이 있는 페이지로 바로 간다 — 화면 밖으로 밀린 세션도 계속 돌고 있고 스크롤백도 그대로다. 칸이 남으면 빈 칸이 드롭 자리로 남는다: 탭을 끌어다 놓으면 그 칸에 들어가고, 패인 헤더를 다른 칸으로 끌면 자리가 바뀐다. 패인을 **가장자리나 모서리로 끌면** Windows 창 스냅처럼 배치 자체가 바뀐다 — 위=전체, 좌·우=2칸의 그 절반, 모서리=2×2의 그 사분면.

**여러 폴더를 한 화면에 (작업공간1/2/3)** — 사이드바 맨 위의 작업공간 행으로 탭을 끌어다 놓으면 그 패인이 담긴다. 폴더가 달라도 상관없고, 원본 폴더의 탭은 그대로 남는다. 작업공간 행을 클릭하면 담아 둔 것들이 자기 레이아웃으로 뜬다. 폴더 뷰와 달리 자동으로 채워지지 않으니, 빼려면 그 패인의 **✕** 를 누르면 된다.

**홈으로 돌아가기** — 좌측 상단 로고를 누르면 현재 선택을 건드리지 않고 홈 대시보드로 이동한다. 세션 모니터·빠른 실행·최근 활동 피드 어디서든 클릭 한 번으로 해당 세션으로 돌아갈 수 있다.

**세션 다루기** — 세션 조작은 **패인 헤더**에 있다. **⏹ 중지** 는 프로세스만 끄고 기록은 남기고(나중에 **⟲ 재개**), **✕ 슬롯 비우기** 는 세션을 살려 둔 채 그 칸만 비운다 — 탭은 남아 있으니 다시 누르면 돌아온다. 세션 자체를 지우려면 **🗑 제거** 를 누른다(확인 후 삭제). 패인 헤더의 ✕ 옆에 있고, 상단 헤더 오른쪽 끝에도 같은 버튼이 있어 지금 포커스된 패인의 세션을 지운다. 패인 헤더 **우클릭 → 제거** 도 그대로 동작한다. 폴더를 클릭하면 그 세션들이 그리드에 올라오고, 스크롤백은 앱이 꺼져 있던 동안 것도 함께 복원된다. 앱 종료로 중단됐던 세션이라면 이 첫 열람에서 대화가 자동으로 재개된다.
