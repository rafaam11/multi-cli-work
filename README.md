...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.0.1-4fb7a4" alt="version 1.0.1">
  <img src="https://img.shields.io/badge/local--only-no%20telemetry-3fb950" alt="local only">
</p>

**Multi CLI Work**는 여러 개의 **Codex · Claude Code · PowerShell 세션을 작업 폴더 단위로 묶어** 한 창에서 굴리는 **로컬 전용 Windows 데스크톱 앱**(Electron)이다.
...
- **CLI 업데이트** — 헤더의 🔧 **Tools** 에서 **Update Claude Code** / **Update Codex**. 홈 디렉토리에서 도는 **폴더에 속하지 않는 터미널**이 열려 `claude update` / `codex update` 출력을 그대로 보여준다. 폴더를 하나도 안 열었어도 쓸 수 있다.
- **레지스트리 자가 복구** — 목록 파일이 깨지면 읽기 전용으로 내려앉고 경고 배너에 **Restore** 버튼이 뜬다. 검증된 `.bak`으로 되돌린다.
- **자동 업데이트** — 위 참조.

## 사용법

**폴더 열기** — 사이드바의 **＋** 로 작업할 폴더를 연다. 한 번 연 폴더는 앱을 재시작해도 목록에 남는다. 더 안 쓰면 우클릭 → **Remove from list**.

**세션 만들기** — 폴더를 고르면 오른쪽 위에 **PowerShell · Claude Code · Codex** 세 버튼이 뜬다. 세션이 하나라도 생기면 그 자리가 **＋ New session** 드롭다운으로 바뀐다. 세션은 그 폴더를 작업 디렉토리로 열린다. 설치되지 않은 CLI는 비활성으로 표시된다.
...
**폴더가 사라졌을 때** — 아이콘이 `📁`에서 `📁✕`로 바뀌고 새 세션·Resume이 막힌다. **⟲ Relink** 로 새 경로를 지정하면 풀린다.
