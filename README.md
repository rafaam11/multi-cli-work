...
- **사이드바 목록 + 슬롯 그리드** — 폴더를 누르면 그 폴더의 세션(종료된 세션의 스크롤백 포함)이 **왼쪽 트리에 행으로** 서고 최근 활동순으로 그리드의 슬롯을 채운다. 배치는 **헤더 줄 왼쪽의 레이아웃 타일**에서 고르며(자동 + 프리셋 8개), **레이아웃이 곧 한 화면의 칸 수**다. 세션이 칸 수보다 많으면 **다음 페이지**로 넘어가고(레이아웃 타일 옆의 **◀ 1/2 ▶**, 또는 사이드바 행을 누르면 그 세션이 있는 페이지로 이동), 적으면 빈 칸이 **드롭 자리**로 남는다. 패인 헤더를 끌어 슬롯끼리 자리를 바꾸고, **✕(슬롯 비우기)** 는 세션을 살려 둔 채 그 칸만 비운다. 기본값 **자동**은 세션 수에 맞춰 1=전체 · 2=좌우 · 3=3열 · 4=3열+우측 2분할 · 5=메인+우측 2×2 · 6=3×2로 배치한다.
- **가장자리로 던져 배치하기** — 패인을 그리드의 **위 가장자리**로 끌면 전체 화면, **좌·우 가장자리**로 끌면 2칸의 그쪽 절반, **네 모서리**로 끌면 2×2의 그 사분면이 된다. Windows의 창 스냅과 같은 조작이고, 결과도 레이아웃 타일에서 손으로 고를 수 있는 배치 그대로다. 끄는 동안에는 대상 영역만 반투명으로 미리 보이고, 실제 배치는 놓는 순간 한 번만 바뀐다.
...
**화면 배치 정하기** — 헤더 줄 왼쪽의 레이아웃 타일에서 **자동**과 프리셋 8개(1칸 · 2칸 좌우 · 3칸 2종 · 4칸 2종 · 5칸 · 6칸) 중 하나를 고른다. 고른 배치는 폴더마다(작업공간마다) 따로 기억된다. 세션이 칸 수보다 많으면 레이아웃 타일 옆의 **◀ 2/3 ▶** 로 페이지를 넘기거나, 사이드바 행을 눌러 그 세션이 있는 페이지로 바로 간다 — 화면 밖으로 밀린 세션도 계속 돌고 있고 스크롤백도 그대로다. 칸이 남으면 빈 칸이 드롭 자리로 남는다: 사이드바 행을 끌어다 놓으면 그 칸에 들어가고, 패인 헤더를 다른 칸으로 끌면 자리가 바뀐다. 패인을 **가장자리나 모서리로 끌면** Windows 창 스냅처럼 배치 자체가 바뀐다 — 위=전체, 좌·우=2칸의 그 절반, 모서리=2×2의 그 사분면.
